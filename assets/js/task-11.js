...
function switchMode(newMode) {
    ...
    console.log('switch mode is run');
    console.log(newMode);
    ...
    switch (newMode) {
        ...
        case 4:
            ...
            if (typeof loadTaskDCanvas === 'function') loadTaskDCanvas();
            break;
    }
...
function updateValuesAndChart() {
    ...
    const solarAngleDisplay = document.getElementById('solarAngleValue');
    if (solarAngleDisplay) solarAngleDisplay.textContent = solarAngle.toFixed(1);
...
function loadTaskDCanvas() {
    ...
    colors.forEach((color, i) => {
        ...
        if (primaryRadius) {
            ...
            ctx.arc(0, 0, primaryRadius, 0, 2 * Math.PI);
            ctx.stroke();
        }
        ...
        if (secondaryRadius) {
            ctx.beginPath();
            ctx.strokeStyle = rainbowColors[i];
            ctx.arc(0, 0, secondaryRadius, 0, 2 * Math.PI);
            ctx.stroke();
        }
    });
...
window.onload = () => {
    ...
    const solarAngleSlider = document.getElementById('sliderSolarAngle');
    if (solarAngleSlider) {
        solarAngleSlider.addEventListener('input', e => {
            solarAngle = parseFloat(e.target.value);
            updateValuesAndChart();
        });
    }
