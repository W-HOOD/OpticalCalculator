...
void main() {
    ...
    if (!u_applyMirror) {
        ...
        vec2 worldPos = (u_matrix * vec3(a_position, 1.0)).xy;

        float worldX = worldPos.x * 300.0;
        float worldY = worldPos.y * 300.0;

        vec2 P = vec2(worldX, worldY);          // Original pixel
        vec2 C = vec2(u_mirrorX, u_mirrorY);    // Mirror center (-200, 0)
        float R = u_radius;                     // Radius (200)
        vec2 F = C + vec2(R * 0.5, 0.0);        // Focal point (on the right of C)

        // Step 1: Horizontal ray from P to mirror surface (go leftward to intersect circle)
        // Calculate point Q on the right half of the circle
        float dy = P.y - C.y;
        float yOffset = dy;
        float xOffset = sqrt(max(R * R - yOffset * yOffset, 0.0));
        vec2 Q = C + vec2(xOffset, yOffset);    // Right side of the circle

        // Step 2: Ray from Q to focal point F
        vec2 dir1 = normalize(F - Q);

        // Step 3: Ray from P through center of curvature C
        vec2 dir2 = normalize(C - P);

        // Step 4: Solve for intersection: Q + t1 * dir1 = P + t2 * dir2
...
function drawArc(ctx, mirrorX, mirrorY, radius, canvasSize, dpr) {
    ...
    ctx.beginPath();
    ctx.arc(px, py, radius, 1.5 * Math.PI, 0.5 * Math.PI, false);
    ctx.stroke();
    ...
    ctx.strokeStyle = '#53074eff';

    ctx.beginPath();
    ctx.moveTo(px, py - radius);
    ctx.lineTo(px + 0.5 * radius, py);
    ctx.stroke();

    ctx.beginPath();
    ctx.moveTo(px + 0.5 * radius, py);
    ...
    // --- Draw parabola of reflection ---
    const f = radius / 2; // focal length
    const a = 1 / (2 * radius);
    const step = 1; // y step in world units
    ...
    ctx.stroke(); // strokeStyle is already set above
...
function checkFocalIntersectionAndDrawWarning(img, scaleFactor) {
    ...
    if (!outside) {
        for (let y = top; y <= bottom; y += 1) {
            ...
            // Check if image is left of the mirror and also outside the semicircle
